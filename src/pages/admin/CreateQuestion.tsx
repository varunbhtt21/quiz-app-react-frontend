...
import { apiService, QuestionType, ScoringType } from '../../services/api';
...
const CreateQuestion = () => {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting }, watch, setValue, reset } = useForm<QuestionFormData>({
    defaultValues: {
      ...
      scoring_type: ScoringType.MANUAL,
      max_word_count: 200
    }
  });
  ...
  const handleQuestionTypeChange = (type: QuestionType) => {
    ...
    if (type === QuestionType.MCQ) {
      setValue('max_word_count', undefined);
      setValue('sample_answer', undefined);
      setValue('scoring_type', undefined);
      setValue('keywords_for_scoring', undefined);
      setScoringKeywords([]);
    } else {
      setValue('option_a', undefined);
      setValue('option_b', undefined);
      setValue('option_c', undefined);
      setValue('option_d', undefined);
      setCorrectOptions([]);
      setValue('scoring_type', ScoringType.MANUAL);
      setValue('max_word_count', 200);
    }
  };
  ...
  const onSubmit = async (data: QuestionFormData) => {
    ...
    try {
      ...
      const createdQuestion = await apiService.createQuestion(questionData);
      
      // If there's an image, upload it
      if (data.image) {
        try {
          await apiService.uploadQuestionImage(createdQuestion.id, data.image);
        } catch (imageError) {
          toast({
            title: "Partial Success",
            description: "Question created successfully, but image upload failed. You can add the image later by editing the question.",
            variant: "destructive"
          });
      ...
      toast({
        title: "Success",
        description: `${questionType === QuestionType.MCQ ? 'MCQ' : 'Long Answer'} question created successfully${data.image ? ' with image' : ''}`
      });
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">Design engaging questions for your students</p>
              </div>
            </div>
          </div>
            ...
            <CardContent className="space-y-4">
              ...
              >
                <div className="flex items-start space-x-3 p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                  <RadioGroupItem value={QuestionType.MCQ} id="mcq" className="mt-1" />
                  <div className="flex-1">
                    <Label htmlFor="mcq" className="text-base font-medium cursor-pointer flex items-center">
                      <CheckCircle className="h-5 w-5 mr-2 text-blue-600" />
                      Multiple Choice Question (MCQ)
                    </Label>
                    <p className="text-sm text-gray-600 mt-1">
                      Traditional multiple choice with 4 options (A, B, C, D). Supports single or multiple correct answers.
                    </p>
                  </div>
                </div>
                
                <div className="flex items-start space-x-3 p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                  <RadioGroupItem value={QuestionType.LONG_ANSWER} id="long_answer" className="mt-1" />
                  <div className="flex-1">
                    <Label htmlFor="long_answer" className="text-base font-medium cursor-pointer flex items-center">
                      <MessageSquare className="h-5 w-5 mr-2 text-green-600" />
                      Long Answer Question
                    </Label>
                    <p className="text-sm text-gray-600 mt-1">
                      Open-ended text response question. Supports manual, keyword-based, or automatic scoring.
                    </p>
                  </div>
                </div>
              </RadioGroup>
            ...
            {questionType === QuestionType.MCQ && (
              <Card className="bg-white shadow-sm border border-gray-200">
                <CardHeader>
                  ...
                  <p className="text-sm text-gray-600">Enter the four answer choices and select the correct option(s)</p>
                </CardHeader>
                <CardContent className="space-y-6">
            ...
            {questionType === QuestionType.LONG_ANSWER && (
              <Card className="bg-white shadow-sm border border-gray-200">
                <CardHeader>
                  ...
                  <p className="text-sm text-gray-600">Configure settings for long answer responses</p>
                </CardHeader>
                <CardContent className="space-y-6">
